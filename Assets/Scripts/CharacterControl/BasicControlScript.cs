...
public class BasicControlScript : MonoBehaviour
{
    ...
    [Tooltip("Jump force added per frame space is held (affects max jump height)")]
    ...
    void Update()
    {
        ...
        {
            jumpStart = Time.time;
            forceVector += Vector3.up * minJumpForce;
            increaseJump = true;
            addForce = true;
        ...
        // Stop increasing jump height when space is released, char hits the ground, or its jumped for the maximum time
        if ((!Input.GetKey(KeyCode.Space) || (isGrounded && Time.time - jumpStart > .05f) || Time.time - jumpStart >= maxJumpTime) && increaseJump)
        {
            increaseJump = false;
